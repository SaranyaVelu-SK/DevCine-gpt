...
const GptSearchBar = () => {
  ...
  return (
    <div className='flex flex-col items-center my-5 '>
      <form className='bg-white/50 p-3 rounded-sm' onSubmit={(e) => e.preventDefault()} >
        <input ref={searchText} type='text' placeholder={languages[lang].placeholder} className='text-white bg-transparent p-5 mr-1.5 w-[600px] border-b-2  border-b-black  focus:outline-none focus:bg-white/60 focus:placeholder-black focus:text-black transition-colors duration-200' />
        <button className='p-5 bg-black ml-1.5 text-white  cursor-pointer button-font font-semibold' onClick={handleSearchSubmit}>{languages[lang].search}</button>
      </form>
